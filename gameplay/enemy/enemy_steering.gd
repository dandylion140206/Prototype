...
func get_desired_velocity(position: Vector2, destination: Vector2, delta: float) -> Vector2:
	...
	steering += _get_wander_direction(seek_direction, delta) * wander_weight

	return steering.limit_length(1.0) * target_speed


func _get_wander_direction(seek_direction: Vector2, delta: float) -> Vector2:
	_wander_time += delta

	var wander_angle := (
		sin(_wander_time * wander_speed + _wander_phase)
		* deg_to_rad(wander_strength)
	)

	return seek_direction.rotated(wander_angle)
