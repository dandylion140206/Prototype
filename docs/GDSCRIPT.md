...
- インデントにはタブを使用する。
- 可読性を優先して明確な理由なしに改行しない。
    - 1 行の文字数が 100 文字を超える、引数が複数あり可読性が落ちるなど、明確な理由がある場合のみ改行する。
    - 引数が複数あることだけを理由に改行せず、1 行で読める式や関数呼び出しは 1 行で記述する。
    - `assert`、`push_error`、`push_warning` は 120 文字を目安とする。
- 関数間には 2 行の空行を置く。
- カンマの後には 1 つの空白を入れる。
- 演算子の前後には 1 つの空白を入れる。
